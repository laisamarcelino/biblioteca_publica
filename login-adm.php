  <?php
  session_start();
  if( isset($_POST['id_adm'])){
    $id_adm = $_POST['id_adm'];
    $senha = $_POST['senha'];
    include_once "conexao.php";
    $con = conecta_mysql();
    $sql = "SELECT id_adm, adm, senha FROM adm WHERE adm='$id_adm' AND senha='$senha'";
    $resultado = mysqli_query($con,$sql);
    if ($resultado) {
      $dados_usuario = mysqli_fetch_assoc($resultado);
      if (isset($dados_usuario["id_adm"])) {
        $_SESSION["adm"] = $dados_usuario["id_adm"];
          ...
          <div class="container-fluid w-100" id="login">
            <div class="row justify-content-center">
              <div class="col-12 col-sm-10 col-md-6 col-lg-5 justify-content-center border border-secondary rounded align-middle pl-5 pr-5 wow slideInRight" data-wow-delay=".3s" id="login-formulario" style="background-color:rgba(255,255,255,0.9);animation-delay:0.3s; animation-name: slideInRight;">
                <h3 class="text-center mt-3 mb-3"> Entrar como bibliotecária(o) </h3>
                <form method="post" action="">

                  <label for="id_adm" style="color:black">Código de identificação: </label>
                  <input name="id_adm" type="text" id="id_adm" required class="form-control rounded" style="height: 45px; text-indent:10px;" autofocus>
                  <br/>
                ...
                <p class="text-center mb-3"><a href="login.php" style="color:#505cfd; font-size:12pt;" id="link-ou">Entrar como leitor</a></p>
