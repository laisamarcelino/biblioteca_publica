...
<body>
    ...
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-xl-8 text-center">
          <h1 class="font-weight-bold wow fadeInLeft">Renovar livro</h1>
  ...
  livros.id_livro, livros.etiqueta, livros.titulo, livros.autor,
  presencial.id_presencial, presencial.data_entrega
  FROM presencial
  JOIN usuarios
      ...
      <div class="container">
        <div class="row justify-content-center border">
          ...
          <form method="post" style="padding: 20px;">
            <center><h5>Renovar livro</h5></center>
            <br>
            <div class="form-row">
              <div class="form-group col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                <label>Título do livro:</label>
                <input type="text" name="titulo" rows="1" class="form-control" value="<?php print "" . $livro["titulo"] . "" ?>" disabled> <br>
                ...
                <input type="text" name="autor" rows="1" class="form-control" value="<?php print "" . $livro["autor"] . "" ?>" disabled>
                <br>
                <label>Nova data de vencimento:</label>
                <?php
                $hoje = date('Y/m/d');
                $nova_data_vencimento = date('Y/m/d', strtotime('+7 days', strtotime($hoje)));
                $data = $nova_data_vencimento;
                ?>
                <input type="text" name="nome" rows="1" class="form-control" value="<?php print "".inverteData($data)."" ?>" disabled>
              </div>
            </div>

            <button type='button' class='btn btn-secondary'><a href='perfil-usuario.php'>Voltar</a></button>

            <input type="submit" class="btn btn-primary" style="margin: 20px 0px 20px 0px;" value="Confirmar">
            <input type='hidden' value='1' name='renovar_livro' />
...
if ( isset($_POST["renovar_livro"]) ){
  if ($con){

    $id_livro = $_POST["id_livro"];
    $id_codigo = $_POST["id_codigo"];
    $id_presencial = $_POST["id_presencial"];
    $hoje = date('Y/m/d');
    $data_final = date('Y/m/d', strtotime('+7 days', strtotime($hoje)));

    $sql = "INSERT INTO renovar (id_presencial, id_codigo, id_livro,	data_renovacao,	data_entrega)
    VALUES ('$id_presencial', '$id_codigo', '$id_livro', '$hoje', '$data_final');";

    $sql2 = "UPDATE renovar SET limite = limite - 1 WHERE  id_codigo = '$id_codigo' AND id_livro = '$id_livro';";

    $sql3 = "UPDATE presencial SET data_entrega='$data_final', limite = limite -1 WHERE id_presencial = '$id_presencial' ";

    //$sql4 = "UPDATE presencial SET data_entrega=$data_final WHERE id_presencial = '$id_presencial';";
